  ...
  var POLICY = {
    ...
    sections: [
      ...
      },
      {
        title: "三、数据处理",
        items: [
          "本地账户信息、设置、日志、截图、OCR 结果、网页语音转写文字以及按 AI 服务保存的问题草稿、回复和历史默认保存在当前浏览器配置中。账户密码只保存加盐摘要，不保存明文。",
          "网页语音功能只在用户主动开始后获取当前 Edge 标签页播放的声音，不读取麦克风；原始录音只在内存中用于本地 Whisper 转写，识别完成或取消后释放，不保存为文件，也不上传到项目服务器。",
          "Developer Mode 的 SDK 草稿、隔离存储和能力授权保存在本地；扩展声明 unlimitedStorage，仅用于移除扩展本地存储的固定总配额，使多个 SDK 脚本可分别使用最多 5 MiB 隔离存储，不会授予任意磁盘文件读取或网络访问能力。运行令牌、页面确认和会话只保存在浏览器会话存储中。运行令牌和会话不按固定时间到期，但会在用户停止会话、替换或删除脚本、关闭 Developer Mode、撤销授权、页面上下文失效或浏览器会话结束时清理；一次性页面确认仍保留短期安全有效期。SDK 脚本在受限沙箱中运行，只能调用用户明确确认的 WSB 能力。",
      ...
      },
      {
        ...
        items: [
          ...
          "用户应在启动 SDK 或旧兼容脚本前核对代码来源、能力、运行网站和数据发送范围。普通脚本必须声明 dom 基础权限，network 只控制 fetch、XHR、WebSocket 等主动联网；脚本仍可通过 DOM 触发网页资源加载或导航。旧脚本兼容层能力更宽，只应运行可信脚本。",
